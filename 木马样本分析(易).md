...
> 在jsonservice中，核心在于onStart创建了定时器，定时收集数据

## 0x0 定时任务 doTimerTask
...
> 收集用户信息写入文件（文件名列表在dog.java，文件操作在pig.java）,在发送后（网络操作在cat.java）设置flag为flase
...
private void Get(String urlStr){
	//设置网络连接,接受服务器命令
	while(true){
		//读取服务器指令并把对应的flag置为true,部分指令执行需要执行文件、程序和系统等操作,
		//在定时器下一次触发时收集信息
	}
}
```
...
| 指   令     | 功   能                   | 指            | 功   能            |
| ----------- | ------------------------- | ------------- | ------------------ |
| get_time    | 获取时间                  | del_sms       | 删除指定短信       |
| get_user    | 获取手机基本系信息        | send_sms      | 发送短信           |
| get_account | 获取用户账户信息          | open_App      | 启动  App          |
| get_App     | 采集用户安装的所有App信息 | install_apk   | 安装  App          |
| get_contact | 获取联系人数据            | uninstall_App | 卸载  App          |
| get_sms     | 获取短信内容              | open_dlg      | 弹框               |
| get_sdcard  | 获得SD卡的文件目录        | open_web      | 打开网页           |
| upload      | 上传文件                  | screen        | 截屏               |
| download    | 下载文件                  | volume        | 获取文件系统主目录 |
| del_file    | 删除指定文件              |               |                    |
